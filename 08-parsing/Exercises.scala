...
trait Parsers[ParseError, Parser[+_]]:
  ...
  extension [A](p: Parser[A]) 
    def many: Parser[List[A]] =
      p.flatMap(a => p.many.map(b => a :: b)) | succeed(List.empty[A])
  ...
  def manyA: Parser[Int] =
    char('a').many.map(_.length) | succeed(0)

  // Exercise 5
  
  extension [A](p: Parser[A]) 
    def many1: Parser[List[A]] =
      p.flatMap(a => p.many.map(b => a :: b))
    ...
    def or(p2: => Parser[A]): Parser[A] =
      (s: ParseState) =>
        p(s) match
          ...
          case Result.Failure(_, _) => p2(s)
          case slice @ Result.Slice(_) => slice 
  ...
  def regex(r: Regex): Parser[String] =
    (s: ParseState) =>
      ...
      r.findPrefixOf(in) match
        case Some(value) => 
          if s.isSliced then Slice(value.length)
          else Success(value, value.length)
        case None => Failure(s.loc.toError(r.toString), isCommitted=true)
...
class JSONParser[ParseError, Parser[+_]](P: Parsers[ParseError,Parser]):
  ...
  lazy val QUOTED: Parser[String] =
    (char('"') ** regex("""[\d\w\s]*""".r) ** char('"')).map{
      case ((_, res), _) => res }

    // Cleaner alternative by chbr
    // regex(""""([^"]*)"""".r)
    // .map {_.dropRight(1).substring(1)}

  lazy val DOUBLE: Parser[Double] =
    regex("""(\+|-)?\d+(\.\d+((e|E)(\+|-)?\d+)?)?""".r).map(_.toDouble)
  ...
  lazy val jnull: Parser[JSON] =
    regex("null".r).map(_ => JNull)

  lazy val jbool: Parser[JSON] =
    regex("(true|false)".r).map{
  ...
  lazy val jarray: Parser[JArray] =
    (char('[') ** ws ** commaSeparatedVals ** ws ** char(']')).map{
      case ((((_, _), lst), _), _) => JArray(lst.toIndexedSeq)
    }

  lazy val field: Parser[(String,JSON)] =
    (QUOTED ** ws ** char(':') ** ws ** json).map{
      case ((((str, _), _), _), j) => (str, j)
    }
  ...
  lazy val jobject: Parser[JObject] =
    (char('{') ** ws ** commaSeparatedFields ** ws ** char('}')).map{
      case ((((_, _), obj), _), _) => JObject(obj.toMap)
    }

  lazy val json: Parser[JSON] =
    ws.? |* { jnull | jbool | jstring | jnumber | jarray | jobject }
