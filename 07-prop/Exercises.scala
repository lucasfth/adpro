...
lazy val (y, rng3): (Int, RNG) = rng2.nextInt

// Exercise 3

object Exercise_3:
  
  import org.scalacheck.Prop.{forAll as scForAll, forAllNoShrink}
...
object Gen:
  ...
  def choose (start: Int, stopExclusive: Int): Gen[Int] =
    State { rng =>
      ...
      (nextInt % (stopExclusive-start).abs + start, nextRng)
    }

  // Exercise 6

  def unit[A] (a: =>A): Gen[A] = State(rng => (a, rng))

  def boolean: Gen[Boolean] = choose(0, 2).map(_ == 1)

  def double: Gen[Double] = choose(0, Int.MaxValue).map(v => v.toDouble.abs / (Int.MaxValue.toDouble.abs + 1))
  ...
  extension [A](self: Gen[A])
    ...
    def listOfN(n: Int): Gen[List[A]] =
      (0 until n).foldRight(Gen.unit(List.empty[A]))((_, acc) =>
        acc.flatMap( lst =>
          self.flatMap( a =>
            State( rng =>
              (a :: lst, rng)
  ...
  // Exercise 8
    ...
    // listOfN was implemented as an extension method so it will be easy to
    // call listOfN on a random Gen[A]
